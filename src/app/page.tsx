...
import { 
  ...
  Trash2, ChevronRight, Check, Ghost 
} from 'lucide-react'
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (premium === 'true') { setIsPremium(true); setRemaining(999) }
  }, [])
  ...
  const saveToHistory = (conv: string, plat: string, ctx: string, result: AnalysisResult) => {
    const newEntry: SavedConversation = {
      ...
      analysis: result, conversation: conv
    }
  ...
  const loadFromHistory = (saved: SavedConversation) => {
    ...
    setShowHistory(false); setShowMenu(false); setAppState('results')
  }

  const handleAnalyze = async () => {
    if (conversation.trim().length < 20) return
    setIsLoading(true); setAppState('analyzing')
    ...
    try {
      ...
      if (data.success && data.analysis) {
        ...
        alert('Erreur: ' + (data.error || 'Inconnue')); setAppState('home')
      }
    } catch (error) {
      alert('Erreur réseau'); setAppState('home')
  ...
  const activatePremium = () => {
    setIsPremium(true); setRemaining(999)
        ...
        <div className="p-4">
          ...
          {!isPremium && (
            <button onClick={() => { setShowPremium(true); setShowMenu(false) }}
              className="w-full flex items-center gap-3 p-3 hover:bg-gray-50 rounded-xl mt-1">
              <Crown className="w-5 h-5 text-yellow-500" />
              <div className="text-left flex-1">
                <p className="font-medium">Premium</p>
                <p className="text-xs text-gray-400">4€/mois - Illimité</p>
              </div>
              <ChevronRight className="w-4 h-4 text-gray-300" />
            </button>
          )}
        </div>
        <div className="absolute bottom-4 left-4 text-xs text-gray-400">Version 1.0.0</div>
      </div>
    </div>
  )
      ...
      <div className="absolute inset-4 bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        <div className="p-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-bold text-lg flex items-center gap-2">
            <History className="w-5 h-5 text-purple-500" /> Historique
          </h2>
          ...
          {savedConversations.length === 0 ? (
            ...
            <div className="space-y-3">
              {savedConversations.map((saved) => (
                <div key={saved.id} className="bg-gray-50 rounded-xl p-3 cursor-pointer hover:bg-gray-100">
                  <div className="flex items-start justify-between">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); deleteFromHistory(saved.id) }}
  ...
  const PremiumModal = () => (
    <div className={`fixed inset-0 z-50 transition-all ${showPremium ? 'visible' : 'invisible'}`}>
      ...
      <div className="absolute inset-4 md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-full md:max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-purple-500 via-pink-500 to-violet-500 p-6 text-white text-center">
        ...
        <div className="p-4 flex gap-2">
          <button onClick={() => setShowPremium(false)}
            className="flex-1 py-3 border border-gray-200 rounded-xl font-medium hover:bg-gray-50">Fermer</button>
          <button onClick={activatePremium}
            className="flex-1 py-3 bg-gradient-to-r from-purple-500 via-pink-500 to-violet-500 text-white rounded-xl font-semibold">Activer</button>
        </div>
      </div>
    </div>
  )
        ...
        <div className="fixed top-0 left-0 right-0 z-40 bg-white/80 backdrop-blur-lg border-b border-gray-100">
          <div className="max-w-lg mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <GhostLogo size={32} />
              <span className="font-bold bg-gradient-to-r from-purple-500 to-pink-500 bg-clip-text text-transparent">GhostMeter</span>
            </div>
        ...
        <div className="pt-20 pb-8 px-4 flex flex-col items-center justify-center min-h-screen">
          ...
          <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-6 border-2 border-purple-200">
            <h2 className="text-lg font-semibold text-center mb-4 flex items-center justify-center gap-2">
              <Sparkles className="w-5 h-5 text-purple-500" /> Colle ta conversation
            </h2>
...
Toi: Je pensais aller au ciné ?"
            ...
            />
            <button onClick={handleAnalyze} disabled={conversation.trim().length < 20 || isLoading || (!isPremium && remaining <= 0)}
              className="w-full py-3 text-white font-semibold rounded-xl bg-gradient-to-r from-purple-500 via-pink-500 to-violet-500 hover:opacity-90 disabled:opacity-50">
              {isLoading ? '⏳ Analyse...' : '✨ Analyser'}
            </button>
            <p className="text-center text-sm text-gray-400 mt-3">
              {isPremium ? <span className="text-purple-500 font-medium">♾️ Analyses illimitées</span> : `🎁 ${remaining}/3 analyses gratuites aujourd'hui`}
            </p>
            {!isPremium && remaining <= 0 && (
              <button onClick={() => setShowPremium(true)} className="w-full mt-2 py-2 border border-purple-200 text-purple-500 rounded-xl text-sm font-medium hover:bg-purple-50">
        ...
        <div className="pt-20 pb-8 p-4">
          <div className="max-w-lg mx-auto">
            ...
            <div className="grid grid-cols-3 gap-2">
              <button onClick={() => { setAnalysis(null); setAppState('home') }} className="py-3 bg-gray-100 rounded-xl font-medium hover:bg-gray-200">
                <RefreshCw className="w-4 h-4 mx-auto mb-1" /> Nouveau
              </button>
              <button onClick={handleShare} className="py-3 bg-gray-100 rounded-xl font-medium hover:bg-gray-200">
                <Share2 className="w-4 h-4 mx-auto mb-1" /> Partager
              </button>
              {!isPremium ? (
                <button onClick={() => setShowPremium(true)} className="py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl font-medium">
